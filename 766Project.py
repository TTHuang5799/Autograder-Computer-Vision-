...
def HarrisCornerDetection(exam_img, gray_exam_img):
    ...
    # Show the image with clearly visualized corners
    #cv2.imshow('Detected Corners', exam_img)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
...
def ReScale_ExamPaper(exam_img, dst, threshold, rmvbg_img):
    ...
    #cv2.imwrite('outputs/corrected_image.png', corrected_image)
...
def Highlight_AnsBoxes(corrected_image, answers_positions: np.ndarray):
    ...
    #gray_temp_image = cv2.cvtColor(corrected_image_color, cv2.COLOR_BGR2GRAY)
    #_, mask = cv2.threshold(gray_temp_image, 1, 255, cv2.THRESH_BINARY)
    #corrected_image_color[:, :, 3][mask == 255] = 255  # Set alpha to 255 where text is visible
    #cv2.imwrite('outputs/corrected_image_color.png', corrected_image_color)
...
def scoredaexam(ans: np.ndarray, corrected_image):
    ...
    for answer, key in zip(d_ans, d_key):
        if answer.lower() == key.lower().replace(" ", ""):
            score += (100 / total)
            ans_list.append(1)
        elif (answer.lower() != key.lower()) and (SequenceMatcher(None, answer, key).ratio() > 0.5):
            ans_list.append(2)
        elif answer == '?':
            ans_list.append(2)
        else:
    ...
    #cv2.imshow('Scored Exam Paper', corrected_image)
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
...
def main():
    ...
    exam_img = cv2.imread('test_math.jpg')
    rmvbg_exam_img = remove(exam_img)
    ...
    RecognizeAnsHandWriting(answer_key)
